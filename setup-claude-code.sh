...
# ============================================================================
step "6" "配置 Brave Search（联网搜索）[可选]"
# ============================================================================

echo "  Claude Code 默认不能联网搜索（公司 API 不支持内置搜索功能）。"
echo "  安装 Brave Search MCP 后，Claude 就可以帮你搜索网页信息。"
echo ""
...
read -p "是否现在配置 Brave Search？(y/n) [跳过按回车]: " setup_brave
setup_brave="${setup_brave:-n}"

if [[ "$setup_brave" == "y" || "$setup_brave" == "Y" ]]; then
    if ! command -v npx &> /dev/null; then
        error "需要 Node.js 才能安装 MCP 工具，请先安装 Node.js"
    else
        read -p "请粘贴你的 Brave Search API Key: " BRAVE_KEY
        if [[ -n "$BRAVE_KEY" ]]; then
            info "正在配置 Brave Search MCP..."

            # 使用 claude mcp add 命令（如果 claude 已安装）
            if command -v claude &> /dev/null; then
                claude mcp add brave-search -s user -e BRAVE_API_KEY="$BRAVE_KEY" -- npx -y @brave/brave-search-mcp-server 2>/dev/null && \
                    success "Brave Search MCP 配置完成" || \
                    warn "claude mcp add 失败，尝试手动配置..."
            fi

            # 验证配置是否写入了 ~/.claude.json
            if grep -q "brave-search" "$HOME/.claude.json" 2>/dev/null; then
                success "Brave Search 已配置到 Claude Code"
            else
                # 手动写入（备用方案）
                warn "自动配置未成功，请稍后在 Claude Code 中手动配置"
                echo "  在 Claude Code 中输入："
                echo "  claude mcp add brave-search -s user -e BRAVE_API_KEY=$BRAVE_KEY -- npx -y @brave/brave-search-mcp-server"
            fi
        else
            warn "未输入 API Key，跳过 Brave Search 配置"
        fi
    fi
else
    info "跳过 Brave Search，你可以之后再配置"
    echo "  配置命令："
    echo "  claude mcp add brave-search -s user -e BRAVE_API_KEY=你的Key -- npx -y @brave/brave-search-mcp-server"
fi
